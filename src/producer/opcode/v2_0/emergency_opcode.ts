...
export class EmergencyOpcode extends Opcode {
    constructor() {
        super('emergency', 0x10);
    }

    decode(data: any) {
        let kpo: any = {
            opcode: constants.OPCODE_EMERGENCY
        };
        try {
            ...
            kpo.emergencies = ocBuf.readUInt32();
            kpo.timestamp = Long.fromValue(ocBuf.readUInt64()).toNumber();
            if (!ocBuf.isEob()) {
    ...
    encode(kpo: any) {
        ...
        ocBuf.writeUInt8(0x10);
        ocBuf.writeUInt16(kpo.sid);
        ocBuf.writeUInt32(kpo.emergencies);
        ocBuf.writeUInt64(kpo.timestamp);
        return ocBuf.buf;
    }
