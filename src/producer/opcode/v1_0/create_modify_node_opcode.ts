...
export class CreateModifyNodeOpcode extends Opcode {
    ...
    decode(data: any) {
        ...
        try {
            ...
            if (kpo.options & constants.CREATEMODIFYNODE_UPDATEURL) {
                kpo.updateUrl = ocBuf.readUtf8String();
            }
    ...
    encode(kpo: any) {
        ...
        if (kpo.options & constants.CREATEMODIFYNODE_UPDATEURL) {
            ocBuf.writeUtf8String(kpo.updateUrl);
        }
        if (kpo.options & constants.CREATEMODIFYNODE_HISTORY) {
    ...
    encodeLength(kpo: any) {
        ...
        if (kpo.options & constants.CREATEMODIFYNODE_UPDATEURL) {
            elen += 2 + Buffer.byteLength(kpo.updateUrl, 'utf8');
        }
        if (kpo.options & constants.CREATEMODIFYNODE_HISTORY) {
            elen += 2;
        }
