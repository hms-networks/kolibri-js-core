...
export class GetHashOpcode extends Opcode {
    constructor() {
        super('getHash', 0x03);
    }
    ...
    decode(data: any) {
        let kpo: any = {
            opcode: constants.OPCODE_GETHASH
        };
        try {
            ...
            kpo.options = ocBuf.readUInt32();
            kpo.path = ocBuf.readUtf8String();
            if (!ocBuf.isEob()) {
    ...
    encode(kpo: any) {
        ...
        ocBuf.writeUInt32(kpo.options);
        ocBuf.writeUtf8String(kpo.path);
        return ocBuf.buf;
    }

    encodeLength(kpo: any) {
        return 9 + Buffer.byteLength(kpo.path);
    }
}
